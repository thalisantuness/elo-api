const produtoRepo = require('../repositories/produtoNovoRepository');
const { Estado } = require('../model/Estado');
const sharp = require('sharp');
const s3 = require('../utils/awsConfig');
const { v4: uuidv4 } = require('uuid');

function ProdutoNovoController() {
  ...
  async function criar(req, res) {
    try {
      const { nome, valor, tipo_comercializacao, tipo_produto, estado_id, foto_principal, fotos_secundarias } = req.body;

      if (!nome || !valor) return res.status(400).json({ error: 'nome e valor são obrigatórios' });

      // Valida estado_id se enviado
      if (estado_id !== undefined && estado_id !== null) {
        const estado = await Estado.findByPk(estado_id);
        if (!estado) {
          return res.status(400).json({ error: 'estado_id inválido: estado não encontrado' });
        }
      }

      const dados = { nome, valor, tipo_comercializacao, tipo_produto, estado_id };
...
module.exports = ProdutoNovoController;
